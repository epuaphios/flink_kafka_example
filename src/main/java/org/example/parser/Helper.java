...
public abstract class Helper {
    ...
    public static String getCode(Object object) throws IllegalAccessException {
        try {
            Field a = object.getClass().getDeclaredField("$atcode");
            a.setAccessible(true);
            Object value = a.get(object);
            return value.toString();
        }
    ...
    public static String getStringCode(Object object) throws NoSuchFieldException, IllegalAccessException {
        // TODO: Move to global or use global if exists
        if (object != null) {
            String value = getCode(object);
            if (value != null && !value.trim().equals("")) {
                if (value.length() > 1000) {
                    return value.substring(0, 1000);
                }
